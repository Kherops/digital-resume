document.addEventListener('DOMContentLoaded', () => {
  ...
  (function MatrixRain() {
    ...
    let running = true;
    let speed = 1; // vitesse initiale = 1
    let raf = null;
    ...
    // expose controls
    window.__MR__ = {
      setSpeed: (s) => { if (Number.isFinite(s) && s > 0) speed = s; },
      getSpeed: () => speed
    };
  })();
  ...
  const fs = {
    ...
    skills: {
      'skills.txt': `- Security: Nmap, masscan, Metasploit, Hydra, John/Hashcat, smbclient/smbmap/enum4linux, SQLi, XSS, Nessus,
...
- Backend: Node.js (Express), PHP (Laravel), REST APIs, MySQL/PostgreSQL, Authentication & Sessions`
  ...
  // === Update Matrix speed based on flags found
  function updateMatrixSpeed() {
    let newSpeed = 1;
    if (foundFlags.size === 1) newSpeed = 1.5;
    else if (foundFlags.size === 2) newSpeed = 2;
    else if (foundFlags.size >= 3) newSpeed = 2.5;

    if (window.__MR__) window.__MR__.setSpeed(newSpeed);
    print(`⚡ Matrix rain speed adjusted: ${newSpeed}`);
  }
  ...
  exit            Quit and open CV`);
    ...
    exit() {
      ...
      const url = "assets/CV%20LOUIS%20PROTON%2019.09.2025.pdf";
        ...
        setTimeout(() => {
          if (i > 0) {
            ...
            if (!win) {
              ...
              link.target = "_blank";
              link.rel = "noopener";
              link.textContent = "[ Curriculum Vitae Louis Proton ]";
